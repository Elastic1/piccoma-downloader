...
export default class PiccomaPage {
  ...
  async getEpisodes(url) {
    ...
    return this.page.evaluate((useFree) => {
      ...
      const results = Array.from(episodes)
        .filter(ep => {
      ...
      if (useFree) {
        const ep = Array.from(episodes)
          .filter(ep => !ep.parentElement.classList.contains('PCM-epList_read'))
          .find(ep => ep.querySelector('.PCM-epList_status_webwaitfree img') != null)
        if (ep != null)
          results.push({
            name: ep.querySelector('.PCM-epList_title h2').textContent,
            id: ep.dataset.episode_id,
