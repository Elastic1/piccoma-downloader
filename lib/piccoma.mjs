...
export default class PiccomaPage {
  ...
  async saveVolume(url, dist, options, progress) {
    this.page.goto(url)
    await this.page.waitForNavigation({ waitUntil: 'domcontentloaded' })
    ...
    for (let i = 0; i < pdata.img.length - 1; i++) {
      ...
      if (fs.existsSync(`${dist}/${i + 1}.${options.format}`)) {
        continue
      }
      const buffer = await this._saveUrlImage(pdata, url, options)
      await fs.promises.writeFile(`${dist}/${i + 1}.${options.format}`, buffer)
      progress(i + 1,　pdata.img.length - 1)
    }
  }

  async _saveUrlImage(pdata, url, options) {
    const image = await this._loadImage(url)
    const canvas = unscramble(pdata, image, 50, getSeed(url))
    if (options.format == 'jpg') {
      return canvas.toBuffer('image/jpeg', { quality: options.quality ?? 85 })
    } else {
  ...
  async _loadImage(url, _image = null, retry = true) {
    const image = _image == null ? new Image() : image
    try {
