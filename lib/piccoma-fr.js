...
export default class PiccomaFr extends Piccoma {
  ...
  async getBookmarks() {
    ...
    for (let i = 1; i <= page; i++) {
      const url = `https://piccoma.com/fr/api/haribo/api/bookshelf/products?bookshelf_type=B&page=${i}`
      const res = await this.client.get(url)
      if (res.data.data.next != null) {
        page = res.data.data.next
      }
      bookmarks.push(...res.data.data.products.map(pr => {
        return {
          id: pr.id,
          url: `https://piccoma.com/fr/product/episode/${pr.id}`,
          title: this._sanatizePath(pr.title),
          webtoon: pr.icon_type == 'U'
        }
      }))
    }
    return bookmarks
