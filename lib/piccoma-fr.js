...
export default class PiccomaFr extends Piccoma {
  ...
  async getEpisodes(id) {
    ...
    const $ = await cheerio.load(res.data)
    const __NEXT_DATA__ = this._getNextData($)
    const episodes = __NEXT_DATA__.props.pageProps.initialState.productHome.productHome.episode_list
    return episodes
  ...
  async _getPdata($) {
    const __NEXT_DATA__ = this._getNextData($)
    const episodeId = __NEXT_DATA__.props.pageProps.initialState.productHome.productHome.recent_episode_id
      ?? __NEXT_DATA__.props.pageProps.initialState.productHome.productHome.recent_volume_id
    const productId = __NEXT_DATA__.props.pageProps.initialState.productHome.productHome.product.id
