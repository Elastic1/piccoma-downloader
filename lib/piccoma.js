...
export default class Piccoma {
  ...
  async getBookmarks() {
    const res = await this.client.get('https://piccoma.com/web/bookshelf/list')
    const params = res.data.data.bookmark.reduce((params, book) => {
      params.append('product_id[]', book.id)
      return params
    }, new URLSearchParams())
    const resProduct = await this.client.post('https://piccoma.com/web/bookshelf/product', params)
    return resProduct.data.data.products.map(product => {
      return {
        id: product.id,
        url: `https://piccoma.com/web/product/${product.id}`,
        title: product.title,
        webtoon: product.smartoon,
      }
    })
  }
